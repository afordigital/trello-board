...
pub fn ColumnContainer(#[prop(into)] column: Column) -> impl IntoView {
    ...
    view! {
        <>
            ...
            // ref={setNodeRef}
            // style={style}
            <div class=uno!["w-[350px] h-fit flex flex-col text-white"]>
                        ...
                        {move || {
                            filtered_cards()
                                ...
                                .then_some(
                                    view! {
                                        <p class=uno![
                                            "bg-slate-7 w-[20px] h-[20px] text-sm text-center rounded-full"
                                        ]>{filtered_cards().len()}</p>
                                    },
                                )
                        }}
                        <button
                ...
                <div class=uno!["flex flex-col gap-y-4"]>
                    ...
                    <div class=uno![
                        "max-h-[500px] h-[500px] overflow-y-auto flex flex-col gap-3 px-1"
                    ]>
                        {move || {
                            filtered_cards()
                                .iter()
