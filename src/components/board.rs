...
use crate::components::{PlusCircle, ColumnContainer};
use crate::models::Id;
...
pub fn KanbanBoard() -> impl IntoView {
    ...
    view! {
        <div
            class=uno!["mx-auto flex flex-1 w-full items-center overflow-x-auto px-[40px]"]>
        <div class=uno!["mx-auto flex"]>
          // <SortableContext
          //   items={columnsId}
          // >
            <div class=uno!["flex gap-x-4 h-[300px] h-fit"]>
              {move || kanban_state.get().columns().map(|column|
                view!{
                <ColumnContainer
                  column=column.clone()
                />
            ...
            <button
              ...
              class=uno!["h-[60px] w-[350px] flex items-center justify-center gap-2 min-w-[350px] cursor-pointer ml-4 rounded-lg bg-mainBackgroundColor border-2 border-columnBackgroundColor"]
            >
              <PlusCircle />
              Add column
            </button>
